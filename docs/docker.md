...
### Подключение к другим серверам в локальной сети (LAN)

Контейнер GitAI может напрямую обращаться к любым другим компьютерам и серверам в вашей локальной сети по их IP-адресу (например, `192.168.1.50`, `10.0.0.5`).

#### 1. Подключение к Ollama / vLLM / LM Studio на другом компьютере в сети:
1. **На компьютере, где запущен Ollama**, разрешите входящие подключения из сети:
   - В Linux / macOS / Windows задайте переменную окружения `OLLAMA_HOST=0.0.0.0:11434` (по умолчанию Ollama слушает только `127.0.0.1`).
2. В файле `.env` на машине с GitAI укажите IP-адрес того компьютера:
   ```env
   LAN_LLM_URL=http://192.168.1.50:11434/v1
   PLANNER_PROVIDER=lan
   PLANNER_MODEL=qwen2.5-coder:14b
   EDITOR_PROVIDER=lan
   EDITOR_MODEL=qwen2.5-coder:14b
   WORKER_PROVIDER=lan
   WORKER_MODEL_A=qwen2.5-coder:14b
   WORKER_MODEL_B=deepseek-coder-v2:16b
   ```
...
#### 2. Подключение к Gitea / GitLab на другом сервере в сети:
В `.env` укажите сетевой адрес вашего Git-сервера:
```env
GITEA_URL=http://192.168.1.50:3000/api/v1
```
